// SPDX-FileCopyrightText: 2014 SAP SE Srdjan Boskovic <srdjan.boskovic@sap.com>
//
// SPDX-License-Identifier: Apache-2.0

const ValueInputHelp = require("../dist").ValueInputHelp;
const setup = require("./setup");

("use strict");

let VI;
beforeAll(async () => {
  const vi = await ValueInputHelp.new(setup.client, setup.shlpApi);
  return (VI = vi);
});

const shlpId = { type: "SH", name: "KREDA" };

describe("SH KREDA", () => {
  test("SH KREDA", async () => {
    expect.assertions(6);

    const desc = (await VI.getShlpDescriptor(shlpId))["SH KREDA"];

    // title
    const title = VI.getShlpTitle(shlpId);
    expect(title).toEqual("Vendors (General)");

    // scrlenmax
    expect(desc.SCRLENMAX).toEqual(21);

    // shlpoutput
    expect(desc.SHLPOUTPUT).toEqual("LIFNR");

    // alloutputs
    expect(desc.ALLOUTPUTS).toEqual([
      "BEGRU",
      "LAND1",
      "LIFNR",
      "MCOD1",
      "MCOD3",
      "PSTLZ",
      "SORTL",
    ]);

    // search parameters
    const params = VI.getShlpParams(shlpId);
    expect(params).toEqual([
      {
        TABNAME: "M_KREDA",
        FIELDNAME: "SORTL",
        LANGU: "E",
        POSITION: 1,
        OFFSET: 6,
        DOMNAME: "CHAR10",
        ROLLNAME: "SORTL",
        CHECKTABLE: "",
        LENG: 10,
        INTLEN: 20,
        OUTPUTLEN: 10,
        DECIMALS: 0,
        DATATYPE: "CHAR",
        INTTYPE: "C",
        REFTABLE: "",
        REFFIELD: "",
        PRECFIELD: "M_KREDA",
        AUTHORID: "",
        MEMORYID: "",
        LOGFLAG: "X",
        MASK: "",
        MASKLEN: "0000",
        CONVEXIT: "",
        HEADLEN: 10,
        SCRLEN1: 10,
        SCRLEN2: 12,
        SCRLEN3: 18,
        FIELDTEXT: "Sort field",
        REPTEXT: "SearchTerm",
        SCRTEXT_S: "SearchTerm",
        SCRTEXT_M: "Search term",
        SCRTEXT_L: "Search term",
        KEYFLAG: "X",
        LOWERCASE: "",
        MAC: "",
        GENKEY: "",
        NOFORKEY: "",
        VALEXI: "",
        NOAUTHCH: "",
        SIGN: "",
        DYNPFLD: "X",
        F4AVAILABL: "",
        COMPTYPE: "E",
        LFIELDNAME: "SORTL",
        LTRFLDDIS: "",
        BIDICTRLC: "",
        OUTPUTSTYLE: "00",
        NOHISTORY: "",
        AMPMFORMAT: "",
        TOPSHLPNAM: "",
        TOPSHLPFLD: "",
        PARVA: "",
        PARTXT: "",
        SHLPOUTPUT: "",
        SHLPSELPOS: "01",
        SHLPLISPOS: "01",
        DEFAULTVAL: "",
      },
      {
        TABNAME: "M_KREDA",
        FIELDNAME: "LAND1",
        LANGU: "E",
        POSITION: 2,
        OFFSET: 26,
        DOMNAME: "LAND1",
        ROLLNAME: "LAND1_GP",
        CHECKTABLE: "",
        LENG: 3,
        INTLEN: 6,
        OUTPUTLEN: 3,
        DECIMALS: 0,
        DATATYPE: "CHAR",
        INTTYPE: "C",
        REFTABLE: "",
        REFFIELD: "",
        PRECFIELD: "LAND1_GP",
        AUTHORID: "",
        MEMORYID: "",
        LOGFLAG: "X",
        MASK: "  X",
        MASKLEN: "0000",
        CONVEXIT: "",
        HEADLEN: 3,
        SCRLEN1: 10,
        SCRLEN2: 12,
        SCRLEN3: 20,
        FIELDTEXT: "Country Key",
        REPTEXT: "Cty",
        SCRTEXT_S: "Country",
        SCRTEXT_M: "Country",
        SCRTEXT_L: "Country",
        KEYFLAG: "",
        LOWERCASE: "",
        MAC: "X",
        GENKEY: "",
        NOFORKEY: "",
        VALEXI: "",
        NOAUTHCH: "",
        SIGN: "",
        DYNPFLD: "X",
        F4AVAILABL: "X",
        COMPTYPE: "D",
        LFIELDNAME: "",
        LTRFLDDIS: "",
        BIDICTRLC: "",
        OUTPUTSTYLE: "00",
        NOHISTORY: "",
        AMPMFORMAT: "",
        TOPSHLPNAM: "",
        TOPSHLPFLD: "",
        PARVA: "",
        PARTXT: "",
        SHLPOUTPUT: "",
        SHLPSELPOS: "02",
        SHLPLISPOS: "02",
        DEFAULTVAL: "",
      },
      {
        TABNAME: "M_KREDA",
        FIELDNAME: "PSTLZ",
        LANGU: "E",
        POSITION: 3,
        OFFSET: 32,
        DOMNAME: "PSTLZ",
        ROLLNAME: "PSTLZ",
        CHECKTABLE: "",
        LENG: 10,
        INTLEN: 20,
        OUTPUTLEN: 10,
        DECIMALS: 0,
        DATATYPE: "CHAR",
        INTTYPE: "C",
        REFTABLE: "",
        REFFIELD: "",
        PRECFIELD: "M_KREDA",
        AUTHORID: "",
        MEMORYID: "",
        LOGFLAG: "X",
        MASK: "",
        MASKLEN: "0000",
        CONVEXIT: "",
        HEADLEN: 10,
        SCRLEN1: 10,
        SCRLEN2: 14,
        SCRLEN3: 14,
        FIELDTEXT: "Postal Code",
        REPTEXT: "PostalCode",
        SCRTEXT_S: "Post.Code",
        SCRTEXT_M: "Postal Code",
        SCRTEXT_L: "Postal Code",
        KEYFLAG: "X",
        LOWERCASE: "",
        MAC: "",
        GENKEY: "",
        NOFORKEY: "",
        VALEXI: "",
        NOAUTHCH: "",
        SIGN: "",
        DYNPFLD: "X",
        F4AVAILABL: "",
        COMPTYPE: "E",
        LFIELDNAME: "PSTLZ",
        LTRFLDDIS: "",
        BIDICTRLC: "",
        OUTPUTSTYLE: "00",
        NOHISTORY: "",
        AMPMFORMAT: "",
        TOPSHLPNAM: "",
        TOPSHLPFLD: "",
        PARVA: "",
        PARTXT: "",
        SHLPOUTPUT: "",
        SHLPSELPOS: "03",
        SHLPLISPOS: "03",
        DEFAULTVAL: "",
      },
      {
        TABNAME: "M_KREDA",
        FIELDNAME: "MCOD3",
        LANGU: "E",
        POSITION: 4,
        OFFSET: 52,
        DOMNAME: "CHAR25",
        ROLLNAME: "MCDK3",
        CHECKTABLE: "",
        LENG: 25,
        INTLEN: 50,
        OUTPUTLEN: 25,
        DECIMALS: 0,
        DATATYPE: "CHAR",
        INTTYPE: "C",
        REFTABLE: "",
        REFFIELD: "",
        PRECFIELD: "M_KREDA",
        AUTHORID: "",
        MEMORYID: "",
        LOGFLAG: "",
        MASK: "",
        MASKLEN: "0000",
        CONVEXIT: "",
        HEADLEN: 25,
        SCRLEN1: 10,
        SCRLEN2: 15,
        SCRLEN3: 20,
        FIELDTEXT: "Search term for matchcode search",
        REPTEXT: "City",
        SCRTEXT_S: "City",
        SCRTEXT_M: "City",
        SCRTEXT_L: "City",
        KEYFLAG: "X",
        LOWERCASE: "",
        MAC: "",
        GENKEY: "",
        NOFORKEY: "",
        VALEXI: "",
        NOAUTHCH: "",
        SIGN: "",
        DYNPFLD: "X",
        F4AVAILABL: "",
        COMPTYPE: "E",
        LFIELDNAME: "MCOD3",
        LTRFLDDIS: "",
        BIDICTRLC: "",
        OUTPUTSTYLE: "00",
        NOHISTORY: "",
        AMPMFORMAT: "",
        TOPSHLPNAM: "",
        TOPSHLPFLD: "",
        PARVA: "",
        PARTXT: "",
        SHLPOUTPUT: "",
        SHLPSELPOS: "04",
        SHLPLISPOS: "04",
        DEFAULTVAL: "",
      },
      {
        TABNAME: "M_KREDA",
        FIELDNAME: "MCOD1",
        LANGU: "E",
        POSITION: 5,
        OFFSET: 102,
        DOMNAME: "CHAR25",
        ROLLNAME: "MCDK1",
        CHECKTABLE: "",
        LENG: 25,
        INTLEN: 50,
        OUTPUTLEN: 25,
        DECIMALS: 0,
        DATATYPE: "CHAR",
        INTTYPE: "C",
        REFTABLE: "",
        REFFIELD: "",
        PRECFIELD: "M_KREDA",
        AUTHORID: "",
        MEMORYID: "",
        LOGFLAG: "",
        MASK: "",
        MASKLEN: "0000",
        CONVEXIT: "",
        HEADLEN: 25,
        SCRLEN1: 10,
        SCRLEN2: 15,
        SCRLEN3: 20,
        FIELDTEXT: "Search term for matchcode search",
        REPTEXT: "Name 1",
        SCRTEXT_S: "Name",
        SCRTEXT_M: "Name",
        SCRTEXT_L: "Name",
        KEYFLAG: "X",
        LOWERCASE: "",
        MAC: "",
        GENKEY: "",
        NOFORKEY: "",
        VALEXI: "",
        NOAUTHCH: "",
        SIGN: "",
        DYNPFLD: "X",
        F4AVAILABL: "",
        COMPTYPE: "E",
        LFIELDNAME: "MCOD1",
        LTRFLDDIS: "",
        BIDICTRLC: "",
        OUTPUTSTYLE: "00",
        NOHISTORY: "",
        AMPMFORMAT: "",
        TOPSHLPNAM: "",
        TOPSHLPFLD: "",
        PARVA: "",
        PARTXT: "",
        SHLPOUTPUT: "",
        SHLPSELPOS: "05",
        SHLPLISPOS: "05",
        DEFAULTVAL: "",
      },
      {
        TABNAME: "M_KREDA",
        FIELDNAME: "LIFNR",
        LANGU: "E",
        POSITION: 6,
        OFFSET: 152,
        DOMNAME: "LIFNR",
        ROLLNAME: "LIFNR",
        CHECKTABLE: "",
        LENG: 10,
        INTLEN: 20,
        OUTPUTLEN: 10,
        DECIMALS: 0,
        DATATYPE: "CHAR",
        INTTYPE: "C",
        REFTABLE: "",
        REFFIELD: "",
        PRECFIELD: "M_KREDA",
        AUTHORID: "",
        MEMORYID: "LIF",
        LOGFLAG: "X",
        MASK: "",
        MASKLEN: "0000",
        CONVEXIT: "ALPHA",
        HEADLEN: 10,
        SCRLEN1: 10,
        SCRLEN2: 15,
        SCRLEN3: 20,
        FIELDTEXT: "Account Number of Vendor or Creditor",
        REPTEXT: "Vendor",
        SCRTEXT_S: "Vendor",
        SCRTEXT_M: "Vendor",
        SCRTEXT_L: "Vendor",
        KEYFLAG: "X",
        LOWERCASE: "",
        MAC: "",
        GENKEY: "",
        NOFORKEY: "",
        VALEXI: "",
        NOAUTHCH: "",
        SIGN: "",
        DYNPFLD: "X",
        F4AVAILABL: "X",
        COMPTYPE: "E",
        LFIELDNAME: "LIFNR",
        LTRFLDDIS: "",
        BIDICTRLC: "",
        OUTPUTSTYLE: "00",
        NOHISTORY: "",
        AMPMFORMAT: "",
        TOPSHLPNAM: "",
        TOPSHLPFLD: "",
        PARVA: "",
        PARTXT: "",
        SHLPOUTPUT: "X",
        SHLPSELPOS: "06",
        SHLPLISPOS: "06",
        DEFAULTVAL: "",
      },
      {
        TABNAME: "M_KREDA",
        FIELDNAME: "BEGRU",
        LANGU: "E",
        POSITION: 7,
        OFFSET: 172,
        DOMNAME: "BRGRU",
        ROLLNAME: "BRGRU",
        CHECKTABLE: "",
        LENG: 4,
        INTLEN: 8,
        OUTPUTLEN: 4,
        DECIMALS: 0,
        DATATYPE: "CHAR",
        INTTYPE: "C",
        REFTABLE: "",
        REFFIELD: "",
        PRECFIELD: "M_KREDA",
        AUTHORID: "",
        MEMORYID: "",
        LOGFLAG: "X",
        MASK: "",
        MASKLEN: "0000",
        CONVEXIT: "",
        HEADLEN: 4,
        SCRLEN1: 10,
        SCRLEN2: 15,
        SCRLEN3: 21,
        FIELDTEXT: "Authorization Group",
        REPTEXT: "AuGr",
        SCRTEXT_S: "Authoriz.",
        SCRTEXT_M: "Authorization",
        SCRTEXT_L: "Authorization Group",
        KEYFLAG: "",
        LOWERCASE: "",
        MAC: "",
        GENKEY: "",
        NOFORKEY: "",
        VALEXI: "",
        NOAUTHCH: "",
        SIGN: "",
        DYNPFLD: "X",
        F4AVAILABL: "",
        COMPTYPE: "E",
        LFIELDNAME: "BEGRU",
        LTRFLDDIS: "",
        BIDICTRLC: "",
        OUTPUTSTYLE: "00",
        NOHISTORY: "",
        AMPMFORMAT: "",
        TOPSHLPNAM: "",
        TOPSHLPFLD: "",
        PARVA: "",
        PARTXT: "",
        SHLPOUTPUT: "",
        SHLPSELPOS: "00",
        SHLPLISPOS: "00",
        DEFAULTVAL: "",
      },
    ]);

    // search result headers
    const headers = VI.getSearchResultHeaders(shlpId);
    expect(headers).toEqual({
      SORTL: ["SearchTerm", 10],
      LAND1: ["Cty", 3],
      PSTLZ: ["PostalCode", 10],
      MCOD3: ["City", 25],
      MCOD1: ["Name 1", 25],
      LIFNR: ["Vendor", 10],
      BEGRU: ["AuGr", 4],
    });
  });

  test("SH KREDA search", async () => {
    expect.assertions(5);

    const sel = [["PSTLZ", "I", "EQ", "69254", ""]];
    const result = await VI.search(shlpId, sel);

    // shlpoutput
    expect(result.shlpoutput).toEqual("LIFNR");

    // maxrows
    expect(result.maxrows_exceeded).toEqual(false);

    // headers
    expect(result.headers).toEqual([
      {
        field: "SORTL",
        len: 10,
        abaptype: "CHAR",
        title: "SearchTerm",
        text_align: "left",
        position: "",
      },
      {
        field: "LAND1",
        len: 3,
        abaptype: "CHAR",
        title: "Cty",
        text_align: "left",
        position: "",
      },
      {
        field: "PSTLZ",
        len: 10,
        abaptype: "CHAR",
        title: "PostalCode",
        text_align: "left",
        position: "",
      },
      {
        field: "MCOD3",
        len: 25,
        abaptype: "CHAR",
        title: "City",
        text_align: "left",
        position: "",
      },
      {
        field: "MCOD1",
        len: 25,
        abaptype: "CHAR",
        title: "Name 1",
        text_align: "left",
        position: "",
      },
      {
        field: "LIFNR",
        len: 10,
        abaptype: "CHAR",
        title: "Vendor",
        text_align: "left",
        position: "",
      },
      {
        field: "BEGRU",
        len: 4,
        abaptype: "CHAR",
        title: "AuGr",
        text_align: "left",
        position: "",
      },
    ]);

    // search result
    expect(result.search_result).toEqual([
      {
        BEGRU: "",
        SORTL: "EMPLOYEE",
        LAND1: "DE",
        PSTLZ: "69254",
        MCOD3: "MALSCH",
        MCOD1: "DOM",
        LIFNR: "YB999101",
      },

      {
        BEGRU: "",
        SORTL: "EMPLOYEE",
        LAND1: "DE",
        PSTLZ: "69254",
        MCOD3: "MALSCH",
        MCOD1: "REIBEL",
        LIFNR: "YB999103",
      },
    ]);

    // search result compact
    const resultCompact = await VI.search(shlpId, sel, { compact: true });
    expect(resultCompact.search_result).toEqual([
      ["", "EMPLOYEE", "DE", "69254", "MALSCH", "DOM", "YB999101"],
      ["", "EMPLOYEE", "DE", "69254", "MALSCH", "REIBEL", "YB999103"],
    ]);
  });
});
