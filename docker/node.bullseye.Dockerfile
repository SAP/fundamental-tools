...
# Build and run
# docker build --platform=linux/amd64 --rm --no-cache -t node.bullseye -f node.bullseye.Dockerfile .
# docker run --platform=linux/amd64  -it --name node.bullseye -v /Users/d037732/SAPDevelop/dev:/home/src node.bullseye /bin/bash --login
